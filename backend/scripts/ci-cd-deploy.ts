...
class CICDDeployer {
    ...
    async deploy(): Promise<void> {
        ...
        if (process.env.GITHUB_ACTIONS) {
            console.log(`::set-output name=graphql-endpoint::${outputs.graphqlEndpoint}`);
            console.log(`::set-output name=identity-pool-id::${outputs.identityPoolId}`);
        }
        ...
        this.execCommand('bun run setup-frontend', 'Generating frontend environment');
